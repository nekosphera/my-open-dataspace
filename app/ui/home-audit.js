(function () {
  ...
  // El conector de este nodo, que es uno. Aqui habia tres identificadores del
  // proyecto de origen y la consulta iba a `/api/connector-1/...`,
  // `/api/connector-2/...` y `/api/connector-3/...`: ninguna de esas rutas
  // existe, asi que el registro de la pagina publica no cargaba nunca.
  // `/api/connector` es el paso al conector, el mismo que usa la consola.
  const CONECTOR = "/api/connector";
  const statusRank = {
    REQUESTED: 10,
    INITIAL: 20,
    STARTED: 30,
  ...
  async function connectorFetchNegotiations() {
    if (state.keycloak) {
      try {
    ...
    const response = await fetch(`${CONECTOR}/management/v3/negotiations`, {
      headers: {
  ...
  async function loadAudit() {
    ...
    const allRows = [];
    const failures = [];
    const settled = await Promise.allSettled([connectorFetchNegotiations()]);
    ...
    settled.forEach((result) => {
      ...
      failures.push(result.reason && result.reason.message ? result.reason.message : "error");
    });
