...
# La carpeta de este fichero, en la ruta de importación. El servicio lo arranca
# como guion —y entonces ya está—, pero las pruebas lo cargan por ruta y ahí no,
# así que `import conectores` fallaba al recolectarlas.
sys.path.insert(0, str(Path(__file__).resolve().parent))

import conectores  # noqa: E402  - después de arreglar la ruta
...
# El registro de operaciones de este nodo. Una linea JSON por evento, en el
# volumen de estado: sobrevive a una actualizacion, que es cuando se
# reconstruye la imagen y se pierde todo lo que viva dentro de ella.
AUDIT_EVENTS_FILE = DATA_DIR / "audit-events.jsonl"
# Un tope, para que el fichero no crezca sin final en un nodo que lleve
# meses encendido. Se conservan los mas recientes, que son los que la
# consola ensena.
AUDIT_EVENTS_MAX = int(os.getenv("ODS_AUDIT_EVENTS_MAX", "5000") or "5000")
...
# El PostgreSQL del nodo, para crear la base de datos de cada conector.
DB_CONTAINER = os.getenv("ODS_DB_CONTAINER", "").strip() or "my-open-dataspace-postgres-1"
DB_USER = os.getenv("ODS_DB_USER", "").strip() or "dataspace"
DB_NAME = os.getenv("ODS_DB_NAME", "").strip() or "dataspace"
...
# Publicar. Es un grupo aparte de `dataspace-admins` a proposito: quien
# publica no tiene por que poder aprobar altas ajenas.
PROVIDER_GROUP = "dataspace-providers"
PROVIDER_GROUP_NAMES = ["dataspace-users", PROVIDER_GROUP]
# Solo quien administra el nodo. Aqui estaba tambien `connector-users`, y a
# ese grupo entra **toda** persona aprobada: cualquiera a quien se diera de
# alta podia aprobar y denegar altas de otros y leer sus datos personales.
REQUEST_REVIEWER_GROUPS = {"dataspace-admins"}
...
def append_audit_event(event):
    """Anade un evento al registro. No revienta a quien lo llama.

    Un fallo escribiendo el registro no puede tumbar la operacion que se
    estaba auditando -- una negociacion cerrada con el registro caido sigue
    siendo una negociacion cerrada -- pero se dice por el log, porque un
    registro que pierde eventos en silencio es peor que no tenerlo.
    """
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        with AUDIT_EVENTS_FILE.open("a", encoding="utf-8") as fh:
            fh.write(json.dumps(event, ensure_ascii=False, sort_keys=True) + "\n")
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[onboarding-api] WARN no se pudo registrar el evento de auditoria: {exc}")
        return False
    trim_audit_events()
    return True


def trim_audit_events():
    """Deja el fichero en AUDIT_EVENTS_MAX lineas, las mas recientes."""
    try:
        if not AUDIT_EVENTS_FILE.exists():
            return
        lineas = AUDIT_EVENTS_FILE.read_text(encoding="utf-8").splitlines()
        if len(lineas) <= AUDIT_EVENTS_MAX:
            return
        AUDIT_EVENTS_FILE.write_text(
            "\n".join(lineas[-AUDIT_EVENTS_MAX:]) + "\n", encoding="utf-8"
        )
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[onboarding-api] WARN no se pudo recortar el registro de auditoria: {exc}")


def read_audit_events(limit=200):
    """Los eventos registrados, del mas reciente al mas antiguo."""
    if not AUDIT_EVENTS_FILE.exists():
        return []
    salida = []
    try:
        ...
        print(f"[onboarding-api] WARN no se pudo leer el registro de auditoria: {exc}")
        return []
    for linea in reversed(lineas):
        ...
        try:
            ...
            # Una linea rota no puede esconder las demas.
            continue
        if len(salida) >= limit:
            break
    return salida
...
def audit_event_or_local(payload):
    ...
    """
    try:
        event = sign_audit_event(payload)
    except Exception:
        event = dict(payload)
...
def crear_base_del_conector(connector_id):
    """La base de datos de un conector. Suya, dentro del mismo PostgreSQL.

    Compartir el servidor no es compartir los datos: cada conector tiene la
    suya y no ve las tablas de los demás. Un PostgreSQL por participante
    multiplicaría los contenedores sin separar nada que esto no separe.
    """
    nombre = conectores.nombre_base(connector_id)
    codigo, salida = conectores.ejecutar_en(DB_CONTAINER, [
        "psql", "-U", DB_USER, "-d", DB_NAME, "-tAc",
        f"SELECT 1 FROM pg_database WHERE datname = '{nombre}'",
    ])
    if codigo != 0:
        raise RuntimeError(f"no se pudo consultar las bases: {salida.strip()[:200]}")
    if salida.strip() == "1":
        return False
    codigo, salida = conectores.ejecutar_en(DB_CONTAINER, [
        "psql", "-v", "ON_ERROR_STOP=1", "-U", DB_USER, "-d", DB_NAME,
        "-c", f'CREATE DATABASE "{nombre}" OWNER "{DB_USER}"',
    ])
    if codigo != 0:
        raise RuntimeError(f"no se pudo crear la base {nombre}: {salida.strip()[:200]}")
    return True


def entorno_del_conector(connector_id, etiqueta=""):
    """Con qué arranca el conector de un participante.

    La misma configuración que la del conector del nodo salvo tres cosas: su
    identificador, su base de datos y su nombre. Los roles, la identidad y los
    destinos de descarga son los del nodo: son política del nodo, no de cada
    persona.
    """
    realm = os.getenv("ODS_REALM", "dataspace").strip() or "dataspace"
    publico = default_public_base_url().rstrip("/")
    return {
        "EDC_CONNECTOR_ID": connector_id,
        "EDC_CONNECTOR_NAME": etiqueta or connector_id,
        "EDC_RBAC_WRITE_ROLES": "dataspace-provider,dataspace-admin",
        "EDC_UPSTREAM_PUBLIC_URL": publico,
        "EDC_UPSTREAM_INTERNAL_URL": "http://app:8092",
        "EDC_AUTH_MODE": "keycloak",
        "EDC_API_CONTROL_BASEURL": f"http://{conectores.nombre_contenedor(connector_id)}:8080",
        "EDC_IAM_ENDPOINT": f"http://keycloak:8080/auth/realms/{realm}",
        "KEYCLOAK_JWKS_URL": f"http://keycloak:8080/auth/realms/{realm}/protocol/openid-connect/certs",
        "KEYCLOAK_ISSUER_URL": f"{publico}/auth/realms/{realm}",
        "DATABASE_URL": f"jdbc:postgresql://postgres:5432/{conectores.nombre_base(connector_id)}",
        "DATABASE_USER": DB_USER,
        "DATABASE_PASSWORD": os.getenv("ODS_DB_PASSWORD", ""),
        "EDC_DOWNLOAD_ALLOWED_HOSTS": os.getenv("ODS_DOWNLOAD_ALLOWED_HOSTS", "localhost"),
    }


TABLAS_DEL_CONECTOR = ("assets", "policy_definitions", "contract_definitions")


def migrar_oferta_al_conector(connector_id):
    """Lleva a su conector lo que esa persona publicó cuando se compartía uno.

    Antes de que cada participante tuviera su instancia, todo vivía en la base
    del nodo con una columna que decía de quién era cada fila. Al darle su
    conector, esas filas tienen que irse con él o su oferta desaparece del
    catálogo.

    Se copia primero y se borra después de comprobar que llegó: al revés, un
    fallo a mitad deja a alguien sin lo suyo.
    """
    nombre = conectores.nombre_base(connector_id)
    movidas = {}
    for tabla in TABLAS_DEL_CONECTOR:
        codigo, cuantas = conectores.ejecutar_en(DB_CONTAINER, [
            "psql", "-U", DB_USER, "-d", DB_NAME, "-tAc",
            f"SELECT count(*) FROM {tabla} WHERE connector_id = '{connector_id}'",
        ])
        if codigo != 0 or not cuantas.strip().isdigit() or cuantas.strip() == "0":
            continue
        codigo, salida = conectores.ejecutar_en(DB_CONTAINER, ["sh", "-c",
            f"psql -U {DB_USER} -d {DB_NAME} -c "
            f"\"COPY (SELECT * FROM {tabla} WHERE connector_id = '{connector_id}') TO STDOUT\" "
            f"| psql -v ON_ERROR_STOP=1 -U {DB_USER} -d {nombre} -c \"COPY {tabla} FROM STDIN\"",
        ])
        if codigo != 0:
            raise RuntimeError(f"no se pudo mover {tabla} a {nombre}: {salida.strip()[:200]}")
        codigo, llegaron = conectores.ejecutar_en(DB_CONTAINER, [
            "psql", "-U", DB_USER, "-d", nombre, "-tAc", f"SELECT count(*) FROM {tabla}",
        ])
        if codigo != 0 or llegaron.strip() != cuantas.strip():
            raise RuntimeError(
                f"{tabla}: se copiaron {llegaron.strip()} de {cuantas.strip()}; no se borra nada"
            )
        conectores.ejecutar_en(DB_CONTAINER, [
            "psql", "-U", DB_USER, "-d", DB_NAME, "-c",
            f"DELETE FROM {tabla} WHERE connector_id = '{connector_id}'",
        ])
        movidas[tabla] = int(cuantas.strip())
    return movidas


def levantar_conector(connector_id, etiqueta=""):
    """Deja en pie el conector de un participante. Devuelve (ok, motivo)."""
    if connector_id == CONNECTOR_ID:
        return True, "el del nodo ya está en pie"
    if not conectores.disponible():
        return False, (
            "este nodo no puede levantar contenedores: falta el socket de Docker "
            "en app (volumes: /var/run/docker.sock)"
        )
    try:
        crear_base_del_conector(connector_id)
        hecho = conectores.crear(connector_id, entorno_del_conector(connector_id, etiqueta))
        if not conectores.esperar(connector_id, segundos=90):
            return False, "su conector no llegó a contestar"
        movidas = migrar_oferta_al_conector(connector_id)
        if movidas:
            print(f"[onboarding-api] {connector_id}: se lleva lo suyo {movidas}")
        return True, ("levantado" if hecho["creado"] else "ya estaba")
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        return False, str(exc)[:300]


def levantar_los_conectores_del_registro():
    """Deja en pie el conector de cada participante, en cada arranque.

    Un `docker compose down` se lleva sus contenedores igual que los del nodo,
    y sin esto volverían a existir sólo cuando alguien los aprobara otra vez.
    """
    if not conectores.disponible():
        print("[onboarding-api] WARN sin socket de Docker: los conectores de los "
              "participantes no se pueden levantar")
        return
    for item in list_registered_connectors():
        cid = str(item.get("connectorId", "") or "").strip()
        if not cid or cid == CONNECTOR_ID:
            continue
        ok, motivo = levantar_conector(cid, etiqueta=item.get("email", ""))
        if not ok:
            print(f"[onboarding-api] WARN no se pudo levantar {cid}: {motivo}")
...
def make_connector_id(email):
    """El conector de esta persona dentro del nodo.

    Estable a partir de su direccion: la misma persona vuelve siempre al mismo
    conector aunque su alta se rehaga. Quien instala el nodo se queda con el
    conector del nodo, `CONNECTOR_ID`.
    """
    email = normalize_user_identity(email)
    if not email:
        return CONNECTOR_ID
    if email == normalize_user_identity(admin_email()):
        return CONNECTOR_ID
    return f"connector-{hashlib.sha1(email.encode('utf-8')).hexdigest()[:10]}"


def ensure_connector_id_in_user_profile(token=None):
    """Declara `connector_id` en el perfil de usuario del realm.

    Keycloak 26 trae el perfil declarativo y **descarta los atributos que no
    estan declarados**: el PUT contesta bien y no guarda nada. Sin esto, el
    claim nunca llega al token.

    Se declara este atributo y no se abre la politica de atributos no
    gestionados entera: sólo administración lo ve y lo edita, que es quien lo
    escribe.
    """
    token = token or get_admin_token()
    perfil = kc_request("GET", f"/admin/realms/{REALM_NAME}/users/profile", token=token) or {}
    atributos = perfil.get("attributes") or []
    if any(a.get("name") == "connector_id" for a in atributos):
        return False
    atributos.append({
        "name": "connector_id",
        "displayName": "Conector del participante",
        "multivalued": False,
    ...
    kc_request(
        "PUT", f"/admin/realms/{REALM_NAME}/users/profile", token=token, payload=perfil
    )
    print("[onboarding-api] declarado connector_id en el perfil de usuario")
    return True


def set_connector_id_attribute(user_id, connector_id, token=None):
    """Escribe `connector_id` en la cuenta, que es de donde sale el claim.

    Vuelve a leer lo que ha escrito. Un `PUT` que contesta bien y no guarda es
    exactamente lo que pasaba antes de declarar el atributo en el perfil, y no
    se notaba hasta mirar un token.

    Idempotente: si ya lo tiene, no se toca nada y devuelve False.
    """
    token = token or get_admin_token()
    ruta = f"/admin/realms/{REALM_NAME}/users/{user_id}"
    actual = kc_request("GET", ruta, token=token) or {}
    atributos = dict(actual.get("attributes") or {})
    if atributos.get("connector_id") == [connector_id]:
        return False
    atributos["connector_id"] = [connector_id]
    kc_request("PUT", ruta, token=token, payload={**actual, "attributes": atributos})

    guardado = (kc_request("GET", ruta, token=token) or {}).get("attributes") or {}
...
def keycloak_groups_for_mode(connector_id, value):
    """Los grupos que le tocan a un perfil de participante.

    Proveer va a `dataspace-providers` y no a `dataspace-admins`. Eran el mismo
    grupo, asi que dar de alta a alguien como proveedor lo convertia en
    administrador del nodo: podia aprobar altas ajenas. Publicar y administrar
    son dos cosas distintas y ahora tienen dos grupos distintos.
    """
    mode = normalize_role_mode(value)
...
# Cada cuanto, como poco, se deja sincronizar a mano. Sin esto, pulsar
# «recargar» repetido lanza una ronda de peticiones a cada nodo vecino por cada
# pulsacion. Quien administra el nodo no tiene freno: si esta mirando por que
# no aparece algo, esperar no le ayuda.
FEDERATION_MIN_SECONDS = max(0, int(os.getenv("ODS_FEDERATION_MIN_SECONDS", "5") or 5))
FEDERATION_LAST_MANUAL = {"at": 0.0}
# Una sincronizacion apuntada para cuando se cierre la ventana del freno.
FEDERATION_PENDIENTE = {"programada": False, "lock": threading.Lock()}
...
def consolidated_catalog(lang=""):
    """El catálogo consolidado, resuelto contra Fuseki desde el servidor.

    La consola pedía esto directamente al SPARQL de Fuseki, con sus
    credenciales de administración metidas en la página. Además de no
    funcionar -- nadie enruta /fuseki, y con razón --, cualquiera que abriera
    la consola se llevaba esas credenciales en el HTML.
    """
    consulta = """
      PREFIX edc: <urn:edc:>
      SELECT ?g ?connector ?assetId ?name ?description ?publisher ?license
             ?accessRights ?theme ?keywords ?mediaType ?deliveryMode
             ?policyId ?contractId ?baseUrl
      WHERE {
        GRAPH ?g {
          ?a edc:connector ?connector ;
             edc:assetId ?assetId ;
             edc:name ?name .
          OPTIONAL { ?a edc:description ?description }
          OPTIONAL { ?a edc:publisher ?publisher }
          OPTIONAL { ?a edc:licenseUrl ?license }
          OPTIONAL { ?a edc:accessRights ?accessRights }
          OPTIONAL { ?a edc:theme ?theme }
          OPTIONAL { ?a edc:keywords ?keywords }
          OPTIONAL { ?a edc:mediaType ?mediaType }
          OPTIONAL { ?a edc:deliveryMode ?deliveryMode }
          OPTIONAL { ?a edc:baseUrl ?baseUrl }
          # La politica y el contrato no cuelgan del activo: el federador los
          # escribe como sujetos aparte y los enlaza con edc:asset y
          # edc:contractPolicy. Preguntar por `?a edc:policyId` -- que era lo
          # que habia aqui -- no encontraba nada nunca, asi que **ninguna fila
          # del catalogo traia politica ni contrato** y la consola contestaba
          # «datos de negociacion incompletos» a cada intento de aceptar.
          OPTIONAL {
      ...
      ORDER BY ?connector ?assetId
    """
    crudo, _ = sparql_select(consulta)
    resultados = json.loads(crudo.decode("utf-8"))

    def valor(fila, clave):
        return (fila.get(clave) or {}).get("value", "")

    filas = [
        {
            # De que nodo viene: el grafo con nombre del que sale la fila.
            # Es exacto y no depende de ningun registro. `connector` es el
            # conector del participante que la publico, que no tiene por que ser
            # un nodo.
            "node": valor(f, "g").rsplit("/", 1)[-1],
            "connector": valor(f, "connector"),
            "assetId": valor(f, "assetId"),
            "name": valor(f, "name"),
            "description": valor(f, "description"),
            "publisher": valor(f, "publisher"),
            "license": valor(f, "license"),
            "accessRights": valor(f, "accessRights"),
            "theme": valor(f, "theme"),
            "keywords": valor(f, "keywords"),
            "mediaType": valor(f, "mediaType"),
            "deliveryMode": valor(f, "deliveryMode"),
            "policyId": valor(f, "policyId"),
            "contractId": valor(f, "contractId"),
            "baseUrl": valor(f, "baseUrl"),
        }
        for f in resultados.get("results", {}).get("bindings", [])
    ]

    nodos = {n["id"]: n for n in list_known_nodes()}
    # Como se llama quien ofrece cada cosa. `connector-1ac633f863` no le dice
    # nada a nadie: con un conector por participante, la columna de proveedor
    # tiene que nombrar a la persona o a la organizacion que lo ofrece, que es
    # con quien se va a negociar.
    #
    # El conector del nodo se nombra con la organizacion, que es publica y esta
    # en la portada. El de un participante, con **el identificador de su
    # conector** y nada mas.
    #
    # Llego a ponerse su correo, por ser a quien se escribe para negociar. No
    # puede: el catalogo lo ve cualquiera que entre en el nodo, y la direccion
    # de una persona no se publica para eso. `connector-1ac633f863` identifica
    # con quien se negocia sin decir quien es.
    etiquetas = {CONNECTOR_ID: dataspace_label()}
    for fila in filas:
        # El nodo es el grafo del que sale la fila; el conector, el participante
        # que la publico. Antes se buscaba el conector en la lista de nodos
        # conocidos, y un conector de participante no esta ahi: la oferta de un
        # vecino del propio nodo se tomaba por remota y la consola iba a
        # negociar contra la direccion del dato en vez de contra el conector.
        nodo = nodos.get(fila["node"]) or nodos.get(fila["connector"]) or {}
        fila["nodeLabel"] = nodo.get("label") or fila["node"] or fila["connector"]
        fila["connectorLabel"] = etiquetas.get(fila["connector"]) or fila["connector"]
        fila["local"] = fila["node"] == CONNECTOR_ID or bool(nodo.get("local"))
    return filas
...
def run_federation_now():
    ...
    with FEDERATION_LOCK:
        ...
        try:
            ...
            # El nodo propio se federa por su **catalogo publico**, igual que
            # cualquier otro.
            #
            # Se leia por la API de gestion con el token de la cuenta de
            # servicio, que ve el nodo entero: entraban activos sin contrato y
            # de conectores ya retirados, es decir, cosas que este nodo no
            # ofrece. Lo que un nodo ofrece se decide en un sitio -- el
            # catalogo publico -- y federarse por otro camino garantizaba que
            # los dos se separasen. Se separaron.
            #
            # La direccion es la del propio proceso: el federador corre dentro
            # de este contenedor.
            nodes_payload = [
                {
                    "id": node["id"],
                    "baseUrl": f"http://127.0.0.1:{PORT}" if node.get("local") else node["baseUrl"],
                    "clientId": "",
                    "clientSecret": "",
                    "role": node.get("role", "provider"),
                    # Todos por catalogo publico: ninguno necesita token.
                    "local": False,
                }
...
# A donde puede salir el conector a buscar un dato. Es el mismo valor que
# recibe el conector, y se lee aqui para poder decirlo al publicar en vez de
# dejar que se descubra al descargar.
DOWNLOAD_ALLOWED_HOSTS = [
    h.strip().lower()
    for h in os.getenv("ODS_DOWNLOAD_ALLOWED_HOSTS", "localhost").split(",")
    if h.strip()
]


def download_host_allowed(url):
    """Si el conector podra ir a buscar el dato a esa direccion.

    Misma regla que el conector: el host, o un subdominio suyo.
    """
    try:
    ...
    if not host:
        return False, ""
    for permitido in DOWNLOAD_ALLOWED_HOSTS:
...
def direcciones_de_los_conectores():
    """Dónde vive el conector de cada participante, y el del nodo.

    Del registro, que es lo que dice quién tiene conector en este nodo. El del
    nodo va siempre y va primero: no depende de ninguna cuenta.
    """
    salida = {CONNECTOR_ID: CONNECTOR_MANAGEMENT_URL}
    for item in list_registered_connectors():
        cid = str(item.get("connectorId", "") or "").strip()
        if cid and cid != CONNECTOR_ID:
            salida[cid] = f"http://{conectores.nombre_contenedor(cid)}:8080"
    return salida


def url_del_conector_de(handler):
    """La dirección del conector de quien llama, sacada de su token.

    Cada participante tiene su instancia, así que el paso no puede llevar a
    todos al mismo sitio. Sin `connector_id` en el token se va al del nodo: es
    el caso de la cuenta de servicio que compone el catálogo.
    """
    try:
        suyo = str(decode_bearer_payload(handler).get("connector_id", "") or "").strip()
    except PermissionError:
        suyo = ""
    if not suyo or suyo == CONNECTOR_ID:
        return CONNECTOR_MANAGEMENT_URL
    return f"http://{conectores.nombre_contenedor(suyo)}:8080"


def proxy_to_connector(handler, method, path, body=None):
    suffix = path[len(CONNECTOR_PROXY_PREFIX):].lstrip("/")
    # `/api/connector/<id>/management/...` para dirigirse a un conector
    # concreto, y `/api/connector/management/...` para el propio.
    #
    # Hace falta desde que cada participante tiene su instancia: negociar con
    # un vecino y descargar lo suyo es hablar con **su** conector, no con el de
    # quien llama. Quién puede hacer qué lo sigue decidiendo el conector de
    # destino con el token, que se reenvía tal cual: esto sólo dice a dónde.
    destino_explicito = ""
    primero = suffix.split("/", 1)[0]
    if primero.startswith("connector") and primero != "management":
        destino_explicito = primero
        suffix = suffix[len(primero):].lstrip("/")
    # `management/` delante es opcional. La consola construye sus rutas como
    # `<base>/management/v3/...` porque es la forma de la API de EDC y la que
    # tiene que valer también contra un nodo remoto, donde `<base>` es la URL
    # del otro nodo. Aquí `<base>` es este paso, así que llega con el prefijo.
    # Aceptar sólo `v3/...` hacía que **todas** las llamadas de la consola
    # dieran 404: cargaba, y no podía leer ni publicar nada.
    ...
    # Publicar un activo cuyo dato esta en un sitio al que el conector no puede
    # ir es publicar una oferta que nadie podra descargar. Se dice ahora, con el
    # host y la variable delante, y no cuatro pasos despues.
    if method == "POST" and recurso == "assets" and body:
        try:
            enviado = json.loads(body.decode("utf-8"))
        except Exception:  # noqa: BLE001 - lo valida el conector
            enviado = {}
        destino = ((enviado.get("dataAddress") or {}) if isinstance(enviado, dict) else {}).get("baseUrl")
        if destino:
            permitido, host = download_host_allowed(destino)
            if not permitido:
                return json_response(handler, 400, {
                    "error": "download_host_not_allowed",
                    "host": host,
                    "allowed": DOWNLOAD_ALLOWED_HOSTS,
                    "message": (
                        f"Este nodo no puede ir a buscar datos a «{host}», asi que "
                        "publicarlo dejaria una oferta que nadie puede descargar. "
                        f"Anade «{host}» a ODS_DOWNLOAD_ALLOWED_HOSTS en el .env "
                        "y reinicia el conector: "
    ...
    # A **su** conector, no al del nodo.
    #
    # El paso enruta por el `connector_id` que trae el token: cada participante
    # tiene su instancia y su base, y hablar todos con la del nodo era
    # justamente el conector compartido. Sin claim se va al del nodo, que es lo
    # que hace la cuenta de servicio al componer el catálogo.
    if destino_explicito:
        if destino_explicito not in direcciones_de_los_conectores():
            return json_response(handler, 404, {"error": "unknown_connector", "connector": destino_explicito})
        base_destino = direcciones_de_los_conectores()[destino_explicito]
    else:
        base_destino = url_del_conector_de(handler)
    url = f"{base_destino}/management/{suffix}"
    ...
    # Un contrato nuevo es una oferta nueva para los demas: hasta que no lo hay
    # no se puede negociar, y en cuanto lo hay tiene que verse. Sin esto habia
    # que esperar al federador -- `ODS_FEDERATION_INTERVAL`, cinco minutos por
    # omision -- y quien publicaba no tenia forma de adelantarlo.
    #
    # Va aqui y no en la consola porque aqui pasa todo el mundo: la consola y
    # cualquiera que use la API.
    if method in ("POST", "PUT", "DELETE") and 200 <= status < 300:
        # La oferta de este nodo ha cambiado, asi que lo que hubiera guardado
        # deja de valer. Sin esto el federador leia el catalogo de hasta
        # treinta segundos antes: se creaban dos contratos seguidos y el
        # segundo no llegaba nunca al grafo, aunque la sincronizacion si
        # corriera. Era el caso que hacia pulsar el boton varias veces.
        CATALOG_CACHE["at"] = 0.0
        CATALOG_CACHE["payload"] = None
    if method == "POST" and recurso == "contractdefinitions" and 200 <= status < 300:
        federar_en_segundo_plano()
...
def federar_en_segundo_plano():
    """Sincroniza el catalogo sin hacer esperar a quien pidio la operacion.

    El freno **encola**, no descarta. Descartaba: dos contratos creados en la
    misma ventana y el segundo no se federaba hasta la ronda periodica, cinco
    minutos despues. Quien lo creo no lo veia, daba por hecho que no habia
    salido y volvia a pulsar -- lo que el freno pretendia evitar.

    Encolando, ninguna peticion se pierde y los nodos vecinos siguen sin
    recibir una ronda por pulsacion: como mucho una cada
    `ODS_FEDERATION_MIN_SECONDS`.
    """
    with FEDERATION_PENDIENTE["lock"]:
        espera = FEDERATION_MIN_SECONDS - (time.time() - FEDERATION_LAST_MANUAL["at"])
        if espera <= 0:
            FEDERATION_LAST_MANUAL["at"] = time.time()
            threading.Thread(target=run_federation_now, daemon=True).start()
            return "ahora"
        if FEDERATION_PENDIENTE["programada"]:
            # Ya hay una apuntada para cuando se cierre la ventana: apuntar otra
            # no adelanta nada y multiplica los hilos.
            return "encolada"
        FEDERATION_PENDIENTE["programada"] = True

    def cuando_toque():
        time.sleep(max(0.0, espera))
        with FEDERATION_PENDIENTE["lock"]:
            FEDERATION_PENDIENTE["programada"] = False
            FEDERATION_LAST_MANUAL["at"] = time.time()
        run_federation_now()

    threading.Thread(target=cuando_toque, daemon=True).start()
    return "encolada"
...
def local_catalog(force=False):
    ...
    # De todos los conectores del nodo, no sólo del suyo.
    #
    # Cada participante tiene su instancia y su base: leer sólo la del nodo
    # dejaba fuera del catálogo todo lo que publiquen los demás. Cada oferta se
    # marca con el conector del que sale, que es lo que permite atribuirla.
    catalog["assets"], catalog["policies"], catalog["contracts"] = [], [], []
    for connector_id, base in direcciones_de_los_conectores().items():
        for clave, ruta in (
            ("assets", "assets"),
            ...
            try:
                req = request.Request(f"{base}/management/v3/{ruta}")
                if token:
                    req.add_header("Authorization", f"Bearer {token}")
                req.add_header("Accept", "application/json")
                # Cinco segundos por conector, no veinte: el catálogo pregunta
                # a todos, y uno que no contesta no puede hacer esperar al
                # resto hasta que la federación caduque.
                with request.urlopen(req, timeout=5) as response:
                    raw = response.read()
                    items = json.loads(raw.decode("utf-8")) if raw else []
            except Exception as exc:  # noqa: BLE001 - un conector caído no vacía el resto
                print(f"[onboarding-api] WARN {connector_id} no contestó por {ruta}: {exc}")
                continue
            for item in items if isinstance(items, list) else []:
                if clave == "assets" and isinstance(item.get("properties"), dict):
                    item["properties"]["ods:connectorId"] = connector_id
                catalog[clave].append(item)

    declarar_politica_y_contrato(catalog)
...
def conectores_vigentes():
    """Los conectores que este nodo sigue teniendo, o None si no se sabe.

    El registro de participantes y, sobre todo, las cuentas: una persona a la
    que se acaba de dar de alta tiene su conector en su cuenta antes de que
    nadie reinicie nada, y su oferta tiene que verse ya.

    Devuelve None si la identidad no contesta, y entonces quien llama no filtra:
    vaciar el catalogo porque Keycloak esta caido seria peor que ofrecer de mas,
    y la seccion 3.1 dice que el catalogo nunca bloquea.
    """
    # Las cuentas, y nada mas. El registro de participantes tambien los lista,
    # pero es papeleo que se queda atras: al borrar una cuenta su entrada
    # sobrevive hasta el siguiente arranque, y mientras tanto el nodo seguia
    # anunciando la oferta de alguien que ya no esta. Quien decide si un
    # participante existe es su cuenta.
    vigentes = {CONNECTOR_ID}
    try:
        token = get_admin_token()
        for cuenta in kc_request("GET", f"/admin/realms/{REALM_NAME}/users", token=token) or []:
            suyo = ((cuenta.get("attributes") or {}).get("connector_id") or [""])[0].strip()
            if suyo:
                vigentes.add(suyo)
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[onboarding-api] WARN no se pudo saber que conectores siguen: {exc}")
        return None
    vigentes.discard("")
    return vigentes


def declarar_politica_y_contrato(catalog):
    """Escribe en cada activo la politica y el contrato que lo cubren.

    El enlace ya existe: la definicion de contrato apunta al activo con su
    `assetsSelector` y a la politica con `contractPolicyId`. Lo que faltaba
    era decirlo donde el catalogo lo declara, `ods:policyId` y
    `ods:contractId`, que es lo unico que mira quien lee la oferta desde otro
    nodo.

    No pisa lo que el activo ya traiga: el sembrado los escribe a mano y esos
    mandan.
    """
    por_activo = {}
    for contrato in catalog.get("contracts") or []:
        contract_id = str(contrato.get("id") or contrato.get("@id") or "").strip()
        if not contract_id:
            continue
        policy_id = str(
            contrato.get("contractPolicyId") or contrato.get("accessPolicyId") or ""
        ).strip()
        for criterio in contrato.get("assetsSelector") or []:
            izquierda = str(criterio.get("leftOperand") or criterio.get("operandLeft") or "")
            # EDC escribe el operando de dos formas segun la version; las dos
            # significan «el identificador del activo».
            if izquierda not in ("id", "https://w3id.org/edc/v0.0.1/ns/id"):
                continue
            asset_id = str(criterio.get("rightOperand") or criterio.get("operandRight") or "").strip()
            if asset_id and asset_id not in por_activo:
                por_activo[asset_id] = (contract_id, policy_id)

    # Lo que se difunde es lo que se puede obtener.
    #
    # Un activo sin definicion de contrato no esta ofrecido: nadie puede
    # negociarlo, y el conector lo rechazaria. Publicarlo aqui manda a todos
    # los nodos vecinos una oferta que no existe -- y le pasa a cualquiera que
    # cree el activo y todavia no le haya hecho el contrato, que es un paso
    # intermedio normal de la consola. Se queda visible para su dueno en «Ver
    # mis activos», que lee del conector directamente.
    #
    # Y tampoco se difunde lo de un conector que ya no esta. Retirar un
    # participante no borra lo que publico -- el conector no sabe borrar -- asi
    # que sin esto el nodo seguia anunciando ofertas de alguien con quien ya no
    # se puede negociar.
    vigentes = conectores_vigentes()
    ofrecidos = []
    for activo in catalog.get("assets") or []:
        propiedades = activo.get("properties")
        if not isinstance(propiedades, dict):
            continue
        identificador = str(
            activo.get("@id")
            or activo.get("id")
            or propiedades.get("dct:identifier")
            or ""
        ).strip()
        enlace = por_activo.get(identificador)
        if enlace:
            contract_id, policy_id = enlace
            if not str(propiedades.get("ods:contractId") or "").strip():
                propiedades["ods:contractId"] = contract_id
            if policy_id and not str(propiedades.get("ods:policyId") or "").strip():
                propiedades["ods:policyId"] = policy_id
        duenno = str(propiedades.get("ods:connectorId") or "").strip() or CONNECTOR_ID
        # `vigentes` a None significa que no se ha podido saber quien sigue
        # estando: entonces no se filtra. Un catalogo que se vacia porque la
        # identidad no contesta es peor que uno que ofrece de mas.
        if str(propiedades.get("ods:contractId") or "").strip() and (
            vigentes is None or duenno in vigentes
        ):
            ofrecidos.append(activo)

    catalog["assets"] = ofrecidos
...
def ensure_admin_user(email, password):
    ...
    """
    # Con nombre **y apellido**: a una cuenta incompleta, Keycloak le exige
    # `VERIFY_PROFILE` después de acertar la contraseña, y la consola no sabe
    # completar esa pantalla -- enseña «No se pudo completar el login».
    user_id, created = ensure_keycloak_user(
        email, password, first_name="Admin", last_name=dataspace_label()
    )
    if not user_id:
    ...
    # `dataspace-providers` tambien: publicar dejo de ser cosa de
    # `dataspace-admins`, y sin este grupo quien instala el nodo no podria
    # publicar en su propio conector.
    # El conector del nodo es suyo, y va en su cuenta para llegar al token.
    try:
        set_connector_id_attribute(user_id, CONNECTOR_ID, token)
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[setup] WARN no se pudo fijar el conector de {email}: {exc}")
    for group in ("dataspace-users", "dataspace-negotiators", "dataspace-admins",
                  PROVIDER_GROUP, f"{CONNECTOR_ID}-users"):
        try:
...
<div class=\"container\">
  ...
  <!-- La pagina se genera una vez y la comparte el nodo. Quien la mira lo dice
       su token, asi que estos dos se reescriben al entrar: si no, a un
       participante le diria que su conector es el del nodo y que el dueno es
       otra persona. -->
  <h1 id=\"consoleHeading\">{heading}</h1>
  <p class=\"muted\">{endpoint_label} <strong>{CONNECTOR_PROXY_PREFIX}</strong></p>
  <p class=\"muted\">Owner: <strong id=\"consoleOwner\">{email}</strong></p>
  ...
  <div id=\"operationTabNav\" class=\"tab-nav\" role=\"tablist\" aria-label=\"{'Connector operation areas' if is_en else 'Áreas operativas del conector'}\">
    ...
    <button id=\"operationRequestsBtn\" class=\"tab-btn\" type=\"button\" role=\"tab\" aria-controls=\"operationRequestsPanel\" aria-selected=\"false\" hidden>{'Users and requests' if is_en else 'Usuarios y solicitudes'}</button>
  </div>

  <!-- La pestana de altas. Nace oculta y la ensena console-solicitudes.js a
       quien administra el nodo: para el resto no hay nada que hacer aqui, y
       una pestana que siempre contesta «no puedes» es peor que no estar. -->
  <div id=\"operationRequestsPanel\" class=\"tab-panel\" role=\"tabpanel\" aria-labelledby=\"operationRequestsBtn\" hidden>
    <div class=\"card\">
      <h2>{'Pending requests' if is_en else 'Solicitudes pendientes'}</h2>
      <p class=\"muted\">{'Someone asked to join this node. Approving creates their account and their access; denying leaves no account behind.' if is_en else 'Alguien ha pedido darse de alta en este nodo. Aprobar le crea la cuenta y su acceso; denegar no deja ninguna cuenta.'}</p>
      <p><button id=\"reloadConnectorRequestsBtn\" class=\"secondary\" type=\"button\">{'Reload' if is_en else 'Recargar'}</button></p>
      <div id=\"connectorRequestsStatus\" class=\"status muted\"></div>
      <table id=\"connectorRequestsTable\"></table>
    </div>
    <div class=\"card\">
      <h2>{'Participants' if is_en else 'Participantes'}</h2>
      <p class=\"muted\">{'Who is already registered in this node.' if is_en else 'Quien esta ya dado de alta en este nodo.'}</p>
      <p><button id=\"reloadConnectorDirectoryBtn\" class=\"secondary\" type=\"button\">{'Reload' if is_en else 'Recargar'}</button></p>
      <div id=\"connectorDirectoryStatus\" class=\"status muted\"></div>
      <table id=\"connectorDirectoryTable\"></table>
    </div>
  </div>
  ...
  <div class=\"card\">
    ...
    <p class=\"muted\">{'The URL where the data actually lives. It is what the connector delivers once a contract is closed, so it has to be reachable from this node.' if is_en else 'La dirección donde vive el dato de verdad. Es lo que el conector entrega cuando se cierra un contrato, así que tiene que ser alcanzable desde este nodo.'}</p>
    <form id=\"assetForm\" class=\"grid\">
      ...
      <div><label>{'Name' if is_en else 'Nombre'}</label><input id=\"assetName\" required placeholder=\"{'Air quality 2026' if is_en else 'Calidad del aire 2026'}\" value=\"\"/></div>
      <div><label>{'Description' if is_en else 'Descripción'}</label><input id=\"assetDescription\" required placeholder=\"{'Hourly particulate readings from three stations' if is_en else 'Medidas horarias de partículas en tres estaciones'}\" value=\"\"/></div>
      ...
      <div><label>{'DCAT-AP keywords (comma)' if is_en else 'Keywords DCAT-AP (coma)'}</label><input id=\"assetKeywords\" required placeholder=\"{'air, environment, stations' if is_en else 'aire, medioambiente, estaciones'}\" value=\"\"/></div>
      <div><label>{'DCAT-AP theme' if is_en else 'Tema DCAT-AP'}</label><input id=\"assetTheme\" required placeholder=\"http://publications.europa.eu/resource/authority/data-theme/ENVI\" value=\"\"/></div>
      ...
      <!-- El perfil de metadatos exige `ods:deliveryMode` y solo admite estos
           cuatro valores, asi que se eligen en vez de escribirse. Antes no
           habia campo ninguno y la consola lo enviaba con el prefijo del
           proyecto de origen: no se podia publicar. -->
      <div><label>{'Delivery mode' if is_en else 'Modo de entrega'}</label><select id=\"assetDeliveryMode\">
        <option value=\"download\" selected>{'download - the file is downloaded' if is_en else 'download - el fichero se descarga'}</option>
        <option value=\"api\">{'api - served through an interface' if is_en else 'api - se sirve por una interfaz'}</option>
        <option value=\"streaming\">{'streaming - continuous flow' if is_en else 'streaming - flujo continuo'}</option>
        <option value=\"dashboard\">{'dashboard - viewed, not downloaded' if is_en else 'dashboard - se consulta, no se descarga'}</option>
      </select></div>
  ...
  <div class=\"card\">
    ...
    <form id=\"policyForm\" class=\"grid\">
      <div style=\"grid-column: 1 / -1;\"><label>{'Data asset reused URL' if is_en else 'URL reutilizada del data asset'}</label><input id=\"policySourceUrl\" required readonly placeholder=\"{'Filled in from the data asset you just created' if is_en else 'Se rellena con el data asset que acabas de crear'}\"/></div>
      <div><label>{'Name' if is_en else 'Nombre'}</label><input id=\"policyName\" required value=\"{'Policy C1' if is_en else 'Política C1'}\"/></div>
      <div><label>{'License URL' if is_en else 'URL de Licencia'}</label><input id=\"policyLicenseUrl\" placeholder=\"https://creativecommons.org/licenses/by/4.0/\" value=\"\"/></div>
      <div><label>{'Purpose' if is_en else 'Propósito de uso'}</label><input id=\"policyPurpose\" placeholder=\"{'Air quality analysis' if is_en else 'Análisis de calidad del aire'}\" value=\"\"/></div>
...
<script>
  window.CONNECTOR_CONFIG = {{
    ...
    baseUrl: {json.dumps(CONNECTOR_PROXY_PREFIX)},
    features: {{
...
def write_connector_pages(connector_id, email, role_mode="both"):
    """Escribe la consola del nodo.

    `role_mode` por omision es «both» y no «consumer»: la consola es una sola
    y la comparte el nodo entero, asi que ensena lo que el nodo sabe hacer.
    Generarla con el perfil de la ultima persona aprobada le quitaba la
    pestana de publicar a todo el mundo, incluido quien administra el nodo.
    """
...
def refresh_existing_connector_pages():
    ...
    """
    try:
        # admin_email(), no REQUESTS_MASTER_EMAIL: el segundo es el .env, y la
        # consola acababa diciendo «Owner: admin@ejemplo.invalid» en un nodo
        # cuyo administrador es otra persona.
        write_connector_pages(CONNECTOR_ID, admin_email(), role_mode="both")
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
...
def retire_connector_artifacts(connector_id, token=None):
    """Retira un conector cuya persona ya no existe: papeles y grupo.

    Se llama **solo** cuando la cuenta ya no esta en el realm. El grupo se
    borra aqui y no en otro sitio porque, sin cuenta detras, ya no es el rastro
    de que nadie se dio de alta: esta vacio por construccion.

    Se para y se borra su contenedor. **Su base de datos no se toca**: es lo
    que publicó esa persona, y borrarla sin que nadie lo haya pedido es tirar
    datos. Se queda ahí y vuelve a usarse si se le da de alta otra vez.
    """
    try:
        if conectores.retirar(connector_id):
            print(f"[onboarding-api] retirado el contenedor de {connector_id}")
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[onboarding-api] WARN no se pudo retirar el contenedor de {connector_id}: {exc}")
    retirados = []
    for ruta in (
        ...
        try:
            if ruta.exists():
                ruta.unlink()
                retirados.append(ruta.name)
        except Exception as exc:  # noqa: BLE001 - se dice, no se traga
            print(f"[onboarding-api] WARN no se pudo retirar {ruta.name}: {exc}")
    try:
        token = token or get_admin_token()
        grupos = kc_request("GET", f"/admin/realms/{REALM_NAME}/groups", token=token) or []
        destino = next((g for g in grupos if g.get("name") == f"{connector_id}-users"), None)
        if destino:
            miembros = kc_request(
                "GET", f"/admin/realms/{REALM_NAME}/groups/{destino['id']}/members", token=token
            ) or []
            if miembros:
                # Con gente dentro no se toca: significa que la busqueda por
                # correo fallo, no que no haya nadie.
                print(f"[onboarding-api] WARN {connector_id}-users tiene miembros; no se retira")
            else:
                kc_request(
                    "DELETE", f"/admin/realms/{REALM_NAME}/groups/{destino['id']}", token=token
                )
                retirados.append(f"{connector_id}-users")
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        print(f"[onboarding-api] WARN no se pudo retirar el grupo de {connector_id}: {exc}")
    return retirados


def reconcile_connector_groups():
    """Que cada participante registrado tenga lo suyo, en cada arranque.

    Su identificador de conector en la cuenta -- que es de donde sale el claim
    y sin el que el conector no sabe de quien es cada activo -- y los grupos que
    le corresponden a su perfil.

    Ya no retira nada. Hubo una version que borraba las entradas de registro
    cuyo conector no era el del nodo, cuando la regla era un conector por nodo;
    con un conector por participante eso borraria el de todo el mundo en el
    primer reinicio.
    """
    granted, retired, left = [], [], []
    token = None
    for item in list_registered_connectors():
        ...
        try:
            if token is None:
                token = get_admin_token()
            usuarios = kc_request(
                "GET", f"/admin/realms/{REALM_NAME}/users", token=token,
                query={"username": email, "exact": "true"},
            ) or []
            ...
            continue
        if not usuarios:
            # Un conector cuya persona ya no esta en el realm. No es que le
            # falte acceso: es que no hay a quien darselo, y dejarlo anuncia al
            # espacio de datos un participante que no existe. El del nodo no se
            # retira nunca: no depende de ninguna cuenta.
            if connector_id != CONNECTOR_ID:
                retirados = retire_connector_artifacts(connector_id, token)
                if retirados:
                    retired.append(connector_id)
            continue
        try:
            set_connector_id_attribute(usuarios[0]["id"], connector_id, token)
            ensure_request_access(
                usuarios[0]["id"], connector_id, role_mode_for_roles(item.get("roles", []))
            )
            granted.append(connector_id)
        except Exception as exc:  # noqa: BLE001
            left.append(f"{connector_id}(no se pudo confirmar su acceso: {exc})")

    # Y al reves: una cuenta con conector propio y sin entrada de registro.
    #
    # El registro de participantes es lo que consulta quien federa, asi que sin
    # entrada esa persona no existe para el espacio de datos por mucho que su
    # consola funcione. Pasa con quien se dio de alta antes de que el registro
    # se escribiera, o si una migracion se llevo su entrada por delante.
    conocidos = {
    ...
    try:
        if token is None:
            token = get_admin_token()
        cuentas = kc_request("GET", f"/admin/realms/{REALM_NAME}/users", token=token) or []
    except Exception as exc:  # noqa: BLE001
        cuentas = []
        left.append(f"no se pudieron leer las cuentas: {exc}")
    for cuenta in cuentas:
        suyo = ((cuenta.get("attributes") or {}).get("connector_id") or [""])[0].strip()
        correo = normalize_user_identity(cuenta.get("username") or cuenta.get("email") or "")
        if not suyo or not correo or suyo == CONNECTOR_ID or suyo in conocidos:
            continue
        try:
            grupos = [g.get("name") for g in kc_request(
                "GET", f"/admin/realms/{REALM_NAME}/users/{cuenta['id']}/groups", token=token
            ) or []]
            modo = "both" if PROVIDER_GROUP in grupos and "dataspace-negotiators" in grupos else (
                "provider" if PROVIDER_GROUP in grupos else "consumer"
            )
            write_connector_registration(suyo, correo, "", role_mode=modo)
            upsert_participant_for_connector(suyo, correo, "", role_mode=modo, status="active")
            conocidos.add(suyo)
            granted.append(suyo)
        except Exception as exc:  # noqa: BLE001
            left.append(f"{suyo}(no se pudo registrar: {exc})")

    if granted:
        print(f"[onboarding-api] conectores confirmados: {', '.join(granted)}")
    if retired:
        print(f"[onboarding-api] conectores retirados por no tener cuenta: {', '.join(retired)}")
    if left:
        print(f"[onboarding-api] WARN conectores sin confirmar: {', '.join(left)}")
    return {"granted": granted, "retired": retired, "left": left}
...
def ensure_console_client_urls():
    """El cliente de la consola tiene que apuntar a la direccion publica.

    `redirectUris` relativos se resuelven contra el `rootUrl` del cliente; sin
    el, contra la raiz del servidor de identidad. Con Keycloak sirviendo bajo
    /auth, eso deja como unico destino admitido <publica>/auth/*, y la pagina
    de acceso vuelve a <publica>/login.html: Keycloak contesta «Invalid
    parameter: redirect_uri» y la consola dice «No se pudo completar el
    login», que no se parece en nada a la causa.

    Idempotente: si ya esta puesto no se toca.
    """
    publica = default_public_base_url().rstrip("/")
    if not publica:
        return
    ...
    clientes = kc_request(
        "GET", f"/admin/realms/{REALM_NAME}/clients", token=token,
        query={"clientId": "dataspace-ui"},
    ) or []
    if not clientes:
        print("[onboarding-api] WARN no existe el cliente dataspace-ui")
        return

    cliente = clientes[0]
    quiere_redirect = sorted({f"{publica}/*", "/*"})
    quiere_origins = sorted({publica, "+"})

    if (
        cliente.get("rootUrl") == publica
        and sorted(cliente.get("redirectUris") or []) == quiere_redirect
        and sorted(cliente.get("webOrigins") or []) == quiere_origins
    ):
        return

    kc_request(
        "PUT", f"/admin/realms/{REALM_NAME}/clients/{cliente['id']}", token=token,
        payload={
            **cliente,
            "rootUrl": publica,
            "baseUrl": "/console.html",
            "redirectUris": quiere_redirect,
            "webOrigins": quiere_origins,
        },
    )
    print(f"[onboarding-api] cliente de la consola apuntando a {publica}")


def ensure_no_blocking_required_actions():
    """`VERIFY_PROFILE` y `VERIFY_EMAIL`, apagadas.

    El realm se importa una sola vez, así que un nodo instalado antes de este
    arreglo las tiene encendidas y su administrador no puede entrar. Esto lo
    corrige en cada arranque, que es lo que hace que actualizar sirva de algo.

    Idempotente: si ya están apagadas no se toca nada.
    """
    token = get_admin_token()
    acciones = kc_request(
        "GET", f"/admin/realms/{REALM_NAME}/authentication/required-actions", token=token
    ) or []
    for accion in acciones:
        alias = accion.get("alias", "")
        if alias not in ("VERIFY_PROFILE", "VERIFY_EMAIL"):
            continue
        if not accion.get("enabled"):
            continue
        kc_request(
            "PUT",
            f"/admin/realms/{REALM_NAME}/authentication/required-actions/{alias}",
            token=token,
            payload={**accion, "enabled": False, "defaultAction": False},
        )
        print(f"[onboarding-api] desactivada la accion obligatoria {alias}")


def ensure_provider_group():
    """El rol y el grupo de proveedor, en un realm que ya existe.

    El realm se importa una sola vez, asi que un nodo instalado antes de esto
    no los tiene y dar de alta a un proveedor fallaria al buscar su grupo.
    Idempotente: si ya estan, no se toca nada.
    """
    token = get_admin_token()
    roles = kc_request("GET", f"/admin/realms/{REALM_NAME}/roles", token=token) or []
    if not any(r.get("name") == "dataspace-provider" for r in roles):
        kc_request(
            "POST", f"/admin/realms/{REALM_NAME}/roles", token=token,
            payload={
                "name": "dataspace-provider",
                "description": "Puede publicar productos de datos en el conector del nodo",
            },
        )
        print("[onboarding-api] creado el rol dataspace-provider")

    grupos = kc_request("GET", f"/admin/realms/{REALM_NAME}/groups", token=token) or []
    destino = next((g for g in grupos if g.get("name") == PROVIDER_GROUP), None)
    if not destino:
        kc_request(
            "POST", f"/admin/realms/{REALM_NAME}/groups", token=token,
            payload={"name": PROVIDER_GROUP},
        )
        grupos = kc_request("GET", f"/admin/realms/{REALM_NAME}/groups", token=token) or []
        destino = next((g for g in grupos if g.get("name") == PROVIDER_GROUP), None)
        print(f"[onboarding-api] creado el grupo {PROVIDER_GROUP}")
    if not destino:
        raise RuntimeError(f"no se pudo asegurar el grupo {PROVIDER_GROUP}")

    puestos = kc_request(
        "GET", f"/admin/realms/{REALM_NAME}/groups/{destino['id']}/role-mappings/realm",
        token=token,
    ) or []
    faltan = []
    for nombre in ("dataspace-user", "dataspace-provider"):
        if any(r.get("name") == nombre for r in puestos):
            continue
        rol = kc_request("GET", f"/admin/realms/{REALM_NAME}/roles/{nombre}", token=token)
        if rol:
            faltan.append({"id": rol["id"], "name": rol["name"]})
    if faltan:
        kc_request(
            "POST",
            f"/admin/realms/{REALM_NAME}/groups/{destino['id']}/role-mappings/realm",
            token=token, payload=faltan,
        )
        print(f"[onboarding-api] {PROVIDER_GROUP} recibe {[r['name'] for r in faltan]}")


def ensure_connector_id_claim():
    """El claim `connector_id`, en el realm y en las cuentas que ya estaban.

    El realm se importa una sola vez, asi que un nodo instalado antes de esta
    enmienda no tiene el mapeador y sus tokens no dicen de que conector es
    quien llama. Sin ese dato el conector no puede separar lo de cada
    participante y todo el mundo veria la misma bolsa.

    Rellena tambien el atributo de las cuentas existentes: la de quien instalo
    el nodo se queda con `CONNECTOR_ID` y cada participante con el suyo, sacado
    de su entrada de registro si la tiene y de su direccion si no.

    Idempotente entera: en el segundo arranque no escribe nada.
    """
    token = get_admin_token()
    ensure_connector_id_in_user_profile(token)
    for cliente in kc_request("GET", f"/admin/realms/{REALM_NAME}/clients", token=token) or []:
        if cliente.get("clientId") not in ("dataspace-ui", "edc-connector"):
            continue
        mapeadores = kc_request(
            "GET",
            f"/admin/realms/{REALM_NAME}/clients/{cliente['id']}/protocol-mappers/models",
            token=token,
        ) or []
        if any(m.get("name") == "connector-id" for m in mapeadores):
            continue
        kc_request(
            "POST",
            f"/admin/realms/{REALM_NAME}/clients/{cliente['id']}/protocol-mappers/models",
            ...
            payload={
                ...
                "config": {
                    "user.attribute": "connector_id",
                    "claim.name": "connector_id",
        ...
        print(f"[onboarding-api] claim connector_id anadido a {cliente.get('clientId')}")

    del_registro = {
        normalize_user_identity(item.get("email", "")): str(item.get("connectorId", "") or "").strip()
        for item in list_registered_connectors()
    }
    puestos = 0
    for usuario in kc_request("GET", f"/admin/realms/{REALM_NAME}/users", token=token) or []:
        correo = normalize_user_identity(usuario.get("username") or usuario.get("email") or "")
        if not correo:
            continue
        destino = del_registro.get(correo) or make_connector_id(correo)
        try:
            if set_connector_id_attribute(usuario["id"], destino, token):
                puestos += 1
        except Exception as exc:  # noqa: BLE001 - se dice, no se traga
            print(f"[onboarding-api] WARN no se pudo fijar el conector de una cuenta: {exc}")
    if puestos:
        print(f"[onboarding-api] conector fijado en {puestos} cuenta(s)")
...
# En que punto van las reparaciones de arranque. Lo lee /health, para que
# una comprobacion pueda esperar a que terminen en vez de medir a medias.
...
def run_startup_repairs():
    ...
    problems = []
    try:
        ensure_no_blocking_required_actions()
    except Exception as exc:  # noqa: BLE001 - reported, not swallowed
        problems.append(f"acciones obligatorias: {exc}")
        print(f"[onboarding-api] WARN no se pudieron revisar las acciones obligatorias: {exc}")
    try:
        ensure_console_client_urls()
    except Exception as exc:  # noqa: BLE001 - reported, not swallowed
        problems.append(f"cliente de la consola: {exc}")
        print(f"[onboarding-api] WARN no se pudo apuntar el cliente de la consola: {exc}")
    try:
        ensure_connector_id_claim()
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        problems.append(f"claim del conector: {exc}")
        print(f"[onboarding-api] WARN no se pudo asegurar el claim connector_id: {exc}")
    try:
        ensure_provider_group()
    except Exception as exc:  # noqa: BLE001 - se dice, no se traga
        problems.append(f"grupo de proveedor: {exc}")
        print(f"[onboarding-api] WARN no se pudo asegurar el grupo de proveedor: {exc}")
    try:
...
ADMIN_GROUP = "dataspace-admins"


def belongs_to_this_node(groups):
    """Si esta persona es participante del conector de este nodo.

    Su grupo `<conector>-users` es el hecho, y lo sabe Keycloak. Antes esto se
    resolvia buscando la direccion en el registro de conectores, donde solo
    esta quien tiene una entrada propia: quien se daba de alta aterrizaba en la
    pagina publica.
    """
    esperado = f"{CONNECTOR_ID}-users"
    return any(
        str(g or "").strip().lstrip("/") == esperado for g in (groups or [])
    )


def administers_this_node(groups):
    """Si esta persona gobierna el nodo.

    Keycloak devuelve los grupos con barra delante -- `/dataspace-admins` --
    segun por donde se miren, asi que se comparan sin ella.
    """
    return any(
        str(g or "").strip().lstrip("/") == ADMIN_GROUP for g in (groups or [])
    )
...
def resolve_connector_redirect(user_value, lang, groups=None):
    ...
    if not found and not from_groups and (
        administers_this_node(groups) or belongs_to_this_node(groups)
    ):
        # Quien administra el nodo gobierna su unico conector, aunque no tenga
        # entrada en el registro ni grupo `connector-<id>-users`: el asistente
        # no se la crea. Sin esto, la persona que instala el nodo acertaba la
        # contrasena, volvia a la pagina publica -- que sigue diciendo
        # «Acceder» -- y no habia forma de llegar a la consola. Le pasaba a la
        # primera persona de cada instalacion.
        found = (CONNECTOR_ID, normalize_user_identity(user_value))
    if from_groups:
...
def mensaje_de_solicitud_enviada(lang):
    """Lo que se le dice a quien acaba de solicitar el alta.

    Sin prometer correo. Aqui se decia «recibiras un correo cuando el
    administrador apruebe o deniegue el alta»: la instalacion por omision no
    tiene correo saliente configurado, asi que ese aviso no lo manda nadie y
    quien se registra se queda esperando un mensaje que no existe. Se dice lo
    que si es cierto -- que hay que aprobarlo, y que vuelva a intentar entrar
    -- que ademas es accionable.
    """
    if str(lang or "es").lower().startswith("en"):
        return (
            "Request submitted. Whoever administers this node has to approve it. "
            "No notice is sent: come back and try to sign in once you have been approved."
        )
    return (
        "Solicitud enviada. Tiene que aprobarla quien administra este nodo. "
        "No se envia ningun aviso: vuelve e intenta entrar cuando te hayan aprobado."
    )


def existing_connector_message(lang):
    """Ya tenia cuenta en este nodo."""
    if str(lang or "es").lower().startswith("en"):
        return (
            "You already had an account on this node. Go to the Login page and sign in "
            "with the same email address and the password you already had."
        )
    return (
        "Ya tenias cuenta en este nodo. Ve a la pantalla de Login e inicia sesion con el "
        "mismo correo y la contrasena que ya tenias."
    )
...
def created_connector_message(lang):
    ...
    Ya no dice «tu conector se ha creado». Este nodo tiene **un** conector y
    darse de alta es pasar a ser participante suyo, no tener otro. Decir lo
    contrario es lo que hacia que alguien recien dado de alta esperase poder
    negociar contra los activos del nodo -- y le salieran como propios, porque
    lo son.
    """
    if str(lang or "es").lower().startswith("en"):
        return (
            "Your access to this node's connector is ready. Go to the Login page and sign in "
            "with the same email address and the password you set when you registered."
        )
    return (
        "Ya tienes acceso al conector de este nodo. Ve a la pantalla de Login e inicia sesion "
        "con el mismo correo y la contrasena que pusiste al registrarte."
    )
...
def create_connector_request(email, password, first_name, last_name, requested_role_mode, lang):
    safe_email = normalize_user_identity(email)
    # El conector de esta persona dentro del nodo. Aprobarla lo despliega.
...
def review_connector_request(request_id, approved, reviewer_email, review_reason=""):
    ...
    # Aprobar despliega el conector de esta persona, con el perfil que pidio.
    #
    # Lo que lo hace suyo, en este orden: el identificador en su cuenta -- que
    # viaja firmado en el token y es lo unico que el conector mira para saber de
    # quien es cada activo --, su entrada en el registro de participantes, y los
    # grupos que le dan lo que puede hacer. Si algo falla, la solicitud queda
    # `failed` con el motivo y no se conceden grupos: nunca un grupo suelto.
    try:
        set_connector_id_attribute(user_id, connector_id)
        # Su conector, en pie, antes de darle acceso. Si no llega a levantarse
        # no se le conceden grupos y la solicitud queda `failed` con el motivo:
        # un participante con grupos y sin conector no puede hacer nada y no
        # sabe por qué.
        levantado, motivo = levantar_conector(connector_id, etiqueta=email)
        if not levantado:
            raise RuntimeError(f"no se pudo levantar su conector: {motivo}")
        write_connector_registration(connector_id, email, "", role_mode=role_mode)
        upsert_participant_for_connector(
            connector_id, email, "", role_mode=role_mode, status="active"
        )
...
def keycloak_expected_issuers():
    ...
    for base in (
        ...
        if base:
            ...
            # `parse`, no `urllib.parse`: aqui solo se importa `parse`, asi
            # que `urllib` no existe y esto lanzaba NameError. El `except` de
            # abajo se lo tragaba, de modo que el emisor interno nunca se
            # anadia y nadie se enteraba.
            host = parse.urlparse(base).hostname or ""
            if host:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/v1/participants":
            # Lleva el correo de cada participante del nodo y se servia sin
            # pedir nada. Lo mira quien administra el nodo.
            try:
        ...
        if parsed.path == "/api/v1/nodes/catalog":
            # El catalogo consolidado, ya resuelto. La consola pedia esto
            # directamente al SPARQL de Fuseki con sus credenciales de
            # administracion dentro de la pagina; aqui la consulta la hace el
            # servidor y esas credenciales no salen del contenedor.
            #
            # Si Fuseki no contesta se dice y se sigue: el catalogo
            # consolidado nunca bloquea la operacion, y la consola cae a su
            # vista local.
            try:
                return json_response(
                    self, 200, {"ok": True, "items": consolidated_catalog()}
                )
            except Exception as exc:  # noqa: BLE001 - se dice, no se traga
                return json_response(
                    self, 200,
                    {"ok": False, "items": [], "error": "catalog_unavailable",
                     "detail": str(exc)[:200]},
                )
        ...
        if parsed.path == "/api/onboarding/connectors":
            # El directorio de participantes del nodo: correo, nombre y
            # apellidos de cada uno. Se servia **sin pedir nada** -- ni token ni
            # sesion -- asi que cualquiera que alcanzara el nodo se llevaba los
            # datos personales de todo el mundo. Lo mira quien administra el
            # nodo, que es quien aprueba las altas.
            try:
                assert_request_reviewer(self)
            except PermissionError as exc:
                return json_response(self, 403, {"error": str(exc)})
        ...
        if parsed.path == "/api/audit/events":
            # El sujeto de cada evento es la direccion de quien lo hizo, asi que
            # esto se servia abierto y con el correo de todo el mundo dentro.
            #
            # Quien administra el nodo ve el registro entero -- es lo que hace
            # que el registro sirva para algo -- y cada participante ve lo suyo.
            try:
                claims = decode_bearer_payload(self)
            except PermissionError as exc:
                return json_response(self, 401, {"error": str(exc)})
            params = parse.parse_qs(parsed.query)
            try:
                limite = int((params.get("limit") or ["200"])[0])
            except ValueError:
                limite = 200
            limite = max(1, min(limite, AUDIT_EVENTS_MAX))
            eventos = read_audit_events(limite)
            if not administers_this_node(claims.get("groups", []) or []):
                quien = normalize_user_identity(
                    claims.get("email") or claims.get("preferred_username") or ""
                )
                suyo = str(claims.get("connector_id", "") or "").strip()
                eventos = [
                    evento for evento in eventos
                    if normalize_user_identity(evento.get("subject", "")) == quien
                    or str((evento.get("evidence") or {}).get("connectorId", "")).strip() == suyo
                ]
            return json_response(self, 200, {"ok": True, "items": eventos})
    ...
    def do_POST(self):
        if self.path == "/api/audit/events":
            ...
            # Aqui habia un reenvio a `forward_audit_event`, una funcion que no
            # existe: quedo el hueco al quitar el servicio de gobernanza
            # externo. El `except` de alrededor convertia el NameError en un
            # 502 «audit_forward_failed», de modo que **todos** los eventos de
            # la consola fallaban y parecia un problema de red.
            #
            # El registro vive en este nodo. Se guarda y se contesta con lo que
            # se ha guardado.
            signed_event.setdefault("receivedAt", utc_now())
            if not append_audit_event(signed_event):
                return json_response(self, 500, {"error": "audit_store_failed"})
            return json_response(self, 201, {"ok": True, "item": signed_event})

        try:
            raw = self.rfile.read(int(self.headers.get("Content-Length", "0")))
            # Un POST sin cuerpo es un cuerpo vacio, no un JSON roto. Lo usan
            # las acciones que no llevan parametros -- «actualizar ahora» es
            # una -- y `json.loads("")` las tumbaba con «invalid_json», un
            # mensaje que no se parece en nada a lo que pasaba.
        ...
        if self.path == "/api/v1/nodes/sync":
            # Sincronizar a mano. No espera al intervalo: quien acaba de crear
            # un contrato -- o de dar de alta un nodo -- quiere ver la oferta
            # ahora y no dentro de cinco minutos.
            #
            # Basta con estar dentro. Exigia `dataspace-admins`, y entonces un
            # participante que publicaba lo suyo no podia hacer que se viera:
            # tenia que esperar al federador o pedirselo a quien administra.
            try:
                claims = decode_bearer_payload(self)
            except PermissionError as exc:
                return json_response(self, 401, {"error": str(exc)})
            if not administers_this_node(claims.get("groups", []) or []):
                # Con freno, pero sin perder la peticion: si la ventana esta
                # abierta se apunta y corre en cuanto se cierre.
                cuando = federar_en_segundo_plano()
                return json_response(self, 200, {
                    "ok": True,
                    "lastRunAt": FEDERATION_STATE["lastRunAt"],
                    "summary": FEDERATION_STATE["lastResult"],
                    "scheduled": cuando,
                })
            FEDERATION_LAST_MANUAL["at"] = time.time()
        ...
        return json_response(
            ...
            200,
            {
                ...
                "status": "pending",
                # Se promete correo solo si este nodo tiene con que enviarlo.
                # Sin SMTP configurado -- que es la instalacion por omision --
                # decirle a alguien que espere un correo es mandarle a esperar
                # un mensaje que nadie va a mandar.
                "message": mensaje_de_solicitud_enviada(lang),
            },
        )
...
if __name__ == "__main__":
    ...
    ensure_requests_storage()
    # Lo que el asistente decidio, otra vez encima de lo que diga el .env.
    #
    # deploy/entrypoint.sh genera runtime-config.js **solo desde el entorno**
    # antes de arrancar esto. Un nodo configurado como «PACO» volvia a
    # llamarse «Mi Organizacion» en cada reinicio, con el correo de contacto
    # de ejemplo, y las respuestas del asistente quedaban en site.json sin
    # que nadie las mirara. Se veia entero en cuanto se reiniciaba el nodo.
    render_site_config()
    refresh_existing_connector_pages()
    # Los conectores de los participantes, en pie. Va en un hilo aparte: cada
    # uno tarda en arrancar y el puerto tiene que abrirse ya.
    threading.Thread(target=levantar_los_conectores_del_registro, daemon=True).start()
    migrate_connectors_to_participants()
