...
# Registros donde todavía no se ha publicado nada. El flujo sube a Docker Hub
# sólo si existen sus dos secretos, y no existen: el espacio de nombres
# `myopendataspace` está libre pero vacío.
REGISTROS_VACIOS = ("myopendataspace/",)


def test_el_readme_no_ofrece_una_imagen_que_no_existe():
    """Un `docker run` en la portada que devuelve «not found».

    Es la primera impresión de quien llega, y descubrirlo por un error del
    demonio de Docker es peor que leerlo. El README ofreció durante un rato
    `myopendataspace/my-open-dataspace`, que da 404 porque las imágenes
    salieron sólo a GHCR.
    """
    readme = leer("README.md")
    # `.` no cruza el salto de línea, así que cada `docker run` se mira solo.
    ofrecidas = re.findall(r"docker (?:run|pull).*?([\w./-]+/[\w.-]+)", readme)
    inexistentes = [
        img for img in ofrecidas if any(img.startswith(v) for v in REGISTROS_VACIOS)
    ]
    assert not inexistentes, (
        "el README ofrece imágenes que no están publicadas: "
        + ", ".join(sorted(set(inexistentes)))
    )


def test_donde_no_se_ha_publicado_se_dice():
    """Si un registro sigue vacío, la documentación tiene que decirlo.

    Callarlo deja a quien bifurque el repositorio preguntándose por qué su
    `docker pull` no encuentra nada.
    """
    doc = sin_saltos("docs/publicar.md")
    assert "Docker Hub está pendiente" in doc, (
        "docs/publicar.md no dice que en Docker Hub todavía no hay nada"
    )
