...
def token_de_la_cuenta_de_servicio(base):
    """Un token del conector, acuñado como lo hace el sembrado.

    Devuelve "" si no hay credenciales de administración a mano, que es el
    caso de quien ejecuta esto contra un nodo que no es suyo.
    """
    admin = os.getenv("ODS_KEYCLOAK_ADMIN_PASSWORD", "").strip()
    if not admin:
        # Segundo intento: el .env del árbol, que es donde lo deja install.sh.
    ...
    realm = os.getenv("ODS_REALM", "").strip() or "dataspace"
    auth = f"{base}/auth"

    def forma(url, datos):
        cuerpo = urllib.parse.urlencode(datos).encode("utf-8")
        req = urllib.request.Request(url, method="POST", data=cuerpo)
    ...
    try:
        ...
        clientes = get(f"{auth}/admin/realms/{realm}/clients?clientId=edc-connector")
        secreto = get(
            f"{auth}/admin/realms/{realm}/clients/{clientes[0]['id']}/client-secret"
        )["value"]
        return forma(
            f"{auth}/realms/{realm}/protocol/openid-connect/token",
            {"client_id": "edc-connector", "client_secret": secreto,
             "grant_type": "client_credentials"},
        )["access_token"]
    except Exception:  # noqa: BLE001 - sin token se salta, no se falla
        return ""
...
def main(base):
    ...
    #
    # El criterio de aceptación 5. No estaba comprobado, y por eso pasó
    # inadvertido que la consola se negaba a publicar: `createAsset` exigía un
    # informe de análisis documental que la poda de la fase 2 había quitado.
    # Comprobar sólo el ejemplo sembrado no vale, porque el sembrado publica
    # por la API y no por el camino que usa una persona.
    autorizacion = {}
    if not evaluacion:
        bearer = token_de_la_cuenta_de_servicio(base)
        if not bearer:
            print("  ...   sin credenciales para acuñar un token; se salta publicar")
        else:
            autorizacion = {"Authorization": f"Bearer {bearer}"}

    if not evaluacion and not autorizacion:
        pass
    else:
        sufijo = str(int(time.time()))
        asset_id = f"urn:ods:dataset:prueba-{sufijo}"
        policy_id = f"policy-prueba-{sufijo}"
        contract_id = f"contract-prueba-{sufijo}"

        codigo, cuerpo = pedir(
            f"{base}/api/connector/v3/assets",
            method="POST",
            ...
            payload={
                ...
                "properties": {
                    ...
                    "dct:title": "Producto de prueba",
                    "dct:description": "Publicado por el recorrido completo.",
                    "dct:publisher": "Recorrido completo",
                    "dct:license": "https://creativecommons.org/licenses/by/4.0/",
                    "dct:accessRights": "restricted",
                    "dcat:theme": "http://publications.europa.eu/resource/authority/data-theme/TECH",
                    "dcat:keyword": "prueba",
                    "dcat:mediaType": "text/csv",
                    "ods:deliveryMode": "download",
                    "ods:policyId": policy_id,
                    "ods:contractId": contract_id,
                },
                "dataAddress": {"type": "HttpData", "baseUrl": f"{base}/api/v1/catalog"},
            },
        )
        comprobar("se puede publicar un activo propio", codigo == 200, json.dumps(cuerpo)[:160])

        codigo, cuerpo = pedir(
            f"{base}/api/connector/v3/policydefinitions",
            method="POST",
            headers=autorizacion,
            payload={
                "@id": policy_id,
                "policy": {"permission": [{"action": "use"}], "ods:policyProfile": "ods-odrl-1.0.0"},
            },
        )
        comprobar("se le puede crear su política", codigo == 200, json.dumps(cuerpo)[:160])

        codigo, cuerpo = pedir(
            f"{base}/api/connector/v3/contractdefinitions",
            method="POST",
            headers=autorizacion,
            payload={
                "@id": contract_id,
        ...
        comprobar("y su contrato", codigo == 200, json.dumps(cuerpo)[:160])

        codigo, _ = pedir(
            f"{base}/api/connector/v3/assets/{urllib.parse.quote(asset_id, safe='')}/download"
        )
        comprobar(
            "el producto recién publicado también exige negociación",
            codigo in (401, 403),
            f"contestó {codigo}",
        )
