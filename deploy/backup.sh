...
# La ruta va dentro del `sh -c`. Suelta, Git Bash en Windows la convierte en
# una ruta de Windows antes de que Docker la vea, y `tar` archiva otra cosa
# sin quejarse: una copia que no contiene lo que crees, y que sólo se
# descubre el día que hace falta restaurarla.
docker compose exec -T app sh -c 'tar czf - -C /var/lib/ods .' > "${CARPETA}/estado.tar.gz"

# Y se comprueba que dentro hay algo. Un tar vacío pesa cuarenta y cinco bytes
# y se guarda igual de bien que uno lleno.
if [ "$(gzip -dc "${CARPETA}/estado.tar.gz" 2>/dev/null | wc -c)" -lt 1024 ]; then
  echo "El archivo de estado ha salido prácticamente vacío." >&2
