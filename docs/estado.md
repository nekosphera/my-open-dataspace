...
### La versión 0.1.0, cortada y comprobada

Todos los trabajos en verde. Las tres imágenes en GHCR, **públicas y para
Intel y ARM**, firmadas con la identidad del flujo y con su inventario de
componentes.

**La imagen publicada se descargó y pasó el recorrido completo: 14
comprobaciones.** No la que se construyó en la máquina de desarrollo: la que
construyó CI y la que se bajará cualquiera.

### Dos fallos más, y los dos de forma

**El análisis de vulnerabilidades bloqueaba la publicación, y no debía.**
`docs/publicar.md` decía que no bloquea; el cableado decía
`needs: [imagenes, vulnerabilidades]`. El `exit-code: "0"` que puse sólo evita
que trivy falle *por encontrar algo* — no hace nada por un fallo del trabajo.
Y el trabajo falló por una versión de acción que no existe
(`trivy-action@0.28.0`; sus etiquetas llevan `v`). Resultado: **las tres
imágenes subidas y la publicación saltada**, que es lo peor de los dos mundos.

Ahora `publicacion` depende sólo de `imagenes`, y hay una prueba que falla si
vuelven a divergir el documento y el flujo: si alguien devuelve
`vulnerabilidades` a los `needs`, **o** si el documento deja de decir que no
bloquea.

**El README ofrecía una imagen que no existe.** `myopendataspace/...` en
Docker Hub da 404: el flujo sube ahí sólo si existen sus dos secretos, y no
existen. Un `docker run` en la portada que no resuelve es la primera impresión
de quien llega. La prueba que vigilaba esto comprobaba que el README dijera
«todavía no están publicadas» — al publicarlas la frase desapareció y la
prueba falló, con razón pero por lo contrario de lo que vigilaba. Reescrita
para comprobar lo que importa: **que el README no ofrezca una imagen que no
está publicada**, con la lista de registros vacíos explícita.
...
## 27 de agosto de 2026 — Lo que se ve abriendo un navegador

Se condujo un Chrome de verdad contra `/login.html` y `/console.html`, que es
por donde se usa el producto. Apareció una cadena de fallos que ninguna prueba
de API podía ver, porque todas llamaban a las rutas correctas directamente sin
ejecutar el JavaScript de la página: la consola no podía leer ni publicar nada
—sus llamadas iban a `/api/connector/management/v3/…` y el paso sólo aceptaba
`/api/connector/v3/…`—; quien administra el nodo entraba bien y aterrizaba en
la página pública; no había ninguna pantalla donde aprobar un alta; aprobar
una le quitaba a todo el nodo la pestaña de publicar; el registro de
operaciones llamaba a una función que no existe; y no se podía publicar porque
la consola enviaba un prefijo de metadatos del proyecto de origen. El detalle
está en el CHANGELOG.

**Verificado en el navegador**, con la instalación de dos contenedores en
marcha: alta por el formulario, aprobación desde la consola, acceso de quien
se dio de alta, publicación de un data asset con su política y su contrato,
federación con «actualizar ahora» y aparición en el catálogo consolidado. Son
las dieciséis comprobaciones de `tests/e2e/navegador_alta_y_acceso.py`.

**Sin verificar todavía, y hay que decirlo:** negociar y descargar contra
**otro** nodo desde la consola. El camino entre pares —`requestNegotiation`
contra la dirección del nodo proveedor— es ahora el único que hay, y los
destinos salen del catálogo consolidado, pero eso se prueba con dos nodos en
marcha y aquí sólo había uno. `docs/dos-nodos.md` explica cómo montarlos.

**El conector no sabe borrar un activo.** No hay `DELETE` en su API de
administración, así que lo publicado no se puede retirar. No es nuevo de hoy,
pero salió al intentar limpiar unos activos de prueba: quedan en el nodo de
pruebas local, fuera del catálogo público porque no tienen contrato.

---

## 27 de agosto de 2026, más tarde — Un nodo, un conector

Un segundo participante dado de alta en el nodo no podía negociar: la consola
le marcaba como propios todos los activos. Y lo son. El alta minaba un
identificador de conector por persona, que la sección 4 de la especificación
manda eliminar; detrás no había más que el único EDC del nodo. Se ha quitado:
un alta es entrar en el conector del nodo, y eso es lo que dicen ahora la
página de alta y la respuesta del servidor.

**Negociar es entre nodos.** Es el criterio 8 de la sección 12 y lo explica
`docs/dos-nodos.md`. Un nodo solo no negocia consigo mismo.

Tirando de ese hilo salieron dos permisos mal puestos, los dos corregidos y
comprobados en el navegador: toda persona aprobada podía aprobar y denegar
altas ajenas, y dar de alta a alguien como proveedor lo convertía en
administrador del nodo.

**Verificado en el navegador**, veinte comprobaciones de
`tests/e2e/navegador_alta_y_acceso.py`: además del recorrido de alta, acceso,
aprobación y publicación, que quien tiene perfil de proveedor aterriza en la
consola, publica en el conector del nodo, **no** ve la pantalla de aprobar
altas y recibe un 403 si la pide por su cuenta.

**El color de marca** ya se aplica: la hoja de estilo tenía su paleta escrita
a mano y el valor elegido no lo leía nadie. Los colores de estado no siguen la
marca a propósito.

**Sigue sin verificar** lo mismo que ayer: negociar y descargar contra otro
nodo desde la consola. Necesita dos nodos levantados.

---

## 28 de agosto de 2026 — El recorrido completo, en un solo nodo

Ayer bloqueé la negociación de la oferta propia dándola por correcta. Estaba
mal: el criterio 6 de la sección 12 pide exactamente publicar un producto y,
**desde la vista de consumo del mismo nodo**, negociar su contrato y descargar
el dato. Con el bloqueo, ese criterio no se podía cumplir, ni por quien
publicaba ni por quien se acababa de dar de alta.

Quitado el bloqueo salieron dos cosas que estaban debajo: la consola adivinaba
el dueño de cada recurso por el nombre del identificador —herencia del
despliegue de tres conectores— y devolvía dueños que no existen; y **la
descarga de un producto alojado en el propio nodo no había funcionado nunca**,
porque el conector no puede resolver la URL pública de su propio nodo desde
dentro.

**Verificado en el navegador**, veinticuatro comprobaciones: alta, aprobación,
acceso, publicación con política y contrato, federación, aparición en el
catálogo consolidado, **negociación desde la vista de consumo y descarga del
dato**. Y sigue rechazándose la descarga sin negociación cerrada, comprobado
con una cuenta recién creada: 403.

**Sigue sin verificar** negociar y descargar contra otro nodo desde la consola:
necesita dos nodos levantados.

### Fuera la pestaña «Nodos conocidos»

Retirada de la consola a petición de quien lleva el producto, con su código
intacto: se apaga en `visibleFor.operationNodesPanel` y se vuelve a encender
ahí mismo. Vale para la consola que viaja en el repositorio y para la que cada
nodo genera, porque las dos cargan el mismo `app.js`.

Da de alta otro nodo por la API mientras tanto —`POST /api/v1/nodes`—; está en
`docs/dos-nodos.md`, `docs/personalizacion.md` y el README, los tres corregidos
para no mandar a nadie a una pestaña que ya no existe.

### Las capturas del README

Las dos se toman de un nodo levantado a propósito para eso, con organización y
correo de ejemplo, y se tira al terminar. La del participante no puede salir
del nodo de quien desarrolla: llevaría su correo y el nombre de su
organización a un repositorio público, que es justo lo que la sección 14
prohíbe.

Sacando esa captura salió que **el atributo `hidden` no ocultaba los botones**:
cualquier regla de autor con `display` gana a la del navegador, y todas las
tienen. La pestaña que se acababa de retirar seguía pintándose, y la de aprobar
altas se le venía enseñando a quien no puede usarla. La prueba miraba el
atributo y por eso no lo vio; ahora mira el ancho que ocupa en pantalla.

---

## 28 de agosto de 2026 — Un conector por participante

La especificación decía «un solo conector» y ayer se podó en consecuencia. Hoy
se ha enmendado —sección 16 de la página de Notion— y el producto la sigue: cada
participante autorizado tiene el suyo.

**Verificado en el navegador**, quince comprobaciones de
`tests/e2e/navegador_conector_por_participante.py`: dos personas del mismo nodo,
cada una con su conector en la cabecera de su consola y su correo como dueño;
una publica activo, política y contrato; la otra no los ve entre los suyos pero
sí en el catálogo del espacio de datos, **atribuidos al conector de la primera**;
acepta la política, cierra la negociación y descarga el dato. Y un consumidor
que intenta publicar recibe un 403.

Nada de esto se ve sin abrir un navegador: el identificador viaja como claim
firmado, la consola lo lee para saber quién mira, y el catálogo atribuye cada
oferta a su participante.

**El límite, dicho claro.** Siguen siendo seis contenedores. El conector de cada
participante es una identidad dentro del mismo tiempo de ejecución EDC: comparten
proceso, base y puerto. La separación es de datos y de permisos, no de recursos.
Está en el README y en la D-015.

**Migración.** Un nodo ya instalado se arregla solo al arrancar: el claim entra
en el realm importado y cada cuenta recibe su conector. Comprobado contra el
nodo de desarrollo, que tenía dos personas dentro.

### La oferta se ve cuando nace, no cinco minutos después

Retirar la pestaña de nodos conocidos se llevó por delante «Actualizar ahora»,
que era la única forma de no esperar al federador. Se notó en cuanto un
participante publicó: ni sus vecinos ni él veían su oferta.

Federar es ahora consecuencia de crear el contrato, y lo hace el nodo en el
paso al conector —no la consola—, de modo que vale para cualquier cliente.

La prueba de extremo a extremo lo tapaba: tenía un paso en el que quien
administra el nodo federaba a mano. Se ha quitado, y con él quitado falló hasta
que el arreglo estuvo en el servidor. Una prueba que hace por su cuenta lo que
el producto debería hacer solo no prueba el producto.
