...
Se da de alta el segundo nodo en el primero. **La pestaña «Nodos conocidos»
no se ofrece en esta entrega** —el panel y su código siguen en la consola, pero
no hay botón que lleve a él—, así que se hace por la API, que es exactamente lo
que aquel botón llamaba.

Desde dentro de un contenedor, `localhost` es el contenedor mismo, así que la
dirección que hay que dar es la de la máquina anfitriona:

```bash
# El token sale de la consola: entra en /console.html y, en la consola del
# navegador, `await window.__DATASPACE_ENSURE_FRESH_TOKEN()`.
curl -X POST http://localhost:8080/api/v1/nodes   -H "Authorization: Bearer ${TOKEN}"   -H "Content-Type: application/json"   -d '{"label": "Consorcio Vecino", "baseUrl": "http://host.docker.internal:8081"}'

# Y sin esperar al intervalo de sincronización:
curl -X POST http://localhost:8080/api/v1/nodes/sync -H "Authorization: Bearer ${TOKEN}"
```
...
Y retira el nodo del primero con `DELETE /api/v1/nodes/<id>`, que se lleva su
grafo con él y deja el resto del almacén intacto.
